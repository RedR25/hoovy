...
export function PlaygroundPage() {
  ...
  return (
    <SceneBackdrop variant="soft" scenery={false}>
      <div className="min-h-screen flex flex-col">
        ...
        <header className="px-4 pt-6 pb-3 flex items-center gap-3 max-w-md w-full mx-auto">
          ...
          <h1 className="flex-1 text-center text-2xl font-extrabold text-hoovy-navy" style={{ fontFamily: 'Fredoka' }}>
            My Episodes
          </h1>
          <ScoreBadge score={12} />
        </header>
        ...
        <main className="flex-1 px-4 pb-28 max-w-md w-full mx-auto">
          {isLoading && (
          ...
          {!isLoading && !isError && scenarios && scenarios.length === 0 && (
            <p className="text-center text-hoovy-navy/40 font-bold mt-10">No episodes yet.</p>
          )}
